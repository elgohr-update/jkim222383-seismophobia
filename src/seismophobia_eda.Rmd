...
earthquake_fct %>%  
  head(10) %>% 
  knitr::kable(caption = "Head of Earthquake Data Set")
```
```{r}
earthquake_fct %>%  
  tail(10) %>% 
...
earthquake_fct %>% 
  ...
  summarize(count = n()) %>% 
  knitr::kable(caption="Count of Different Target Levels")
```
```{r - Summary Tables}
earthquake_fct %>%
  is.na() %>%
  colSums() %>%
  knitr::kable(col.names = "Count of NA's in Feature", caption="NA Count by Feature")
```
